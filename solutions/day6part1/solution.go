...
func ParseRaces(lines []string) []Race {
	timesWithoutPrefix, ok := strings.CutPrefix(lines[0], "Time:")
	if !ok {
		panic(errors.New("parsing error"))
	}
	times := utils.ToIntSlice(strings.TrimSpace(timesWithoutPrefix), " ")
	distWithoutPrefix, ok := strings.CutPrefix(lines[1], "Distance:")
	if !ok {
		panic(errors.New("parsing error"))
	}
	dist := utils.ToIntSlice(strings.TrimSpace(distWithoutPrefix), " ")
	var races []Race
	for i := 0; i < len(dist); i++ {
		races = append(races, Race{Time: times[i], Distance: dist[i]})
	}
	return races
}

func countWaysToBeatRecord(race Race) int {
	count := 0
...
func (Solver) Solve(input string) string {
	races := ParseRaces(utils.GetLines(input))
	var counts []int
	for _, race := range races {
		counts = append(counts, countWaysToBeatRecord(race))
