...
func (c Card) winningNumberCount() int {
	sum := 0
	for _, num := range c.yourNumbers {
		for _, num2 := range c.winningNumbers {
			if num == num2 {
				sum++
			}
		}
...
var r = regexp.MustCompile("^Card\\s+(?P<CardNum>\\d+):\\s+(?P<WinningNumbers>[\\d ]+)\\D+(?P<YourNumbers>.*)")
...
func ParseCard(line string) *Card {
	...
	cardNum, err := strconv.Atoi(matches[r.SubexpIndex("CardNum")])
	if err != nil {
		panic(err)
	}
	winningNumbers := utils.ToIntSlice(matches[r.SubexpIndex("WinningNumbers")], " ")
	yourNumbers := utils.ToIntSlice(matches[r.SubexpIndex("YourNumbers")], " ")
	return &Card{
		num:            cardNum,
...
func getScratchCardCount(cards *[]Card, start int, end int, wg *sync.WaitGroup, channel chan int) {
	defer wg.Done()
	c := *cards
	for i := start; i < end; i++ {
		score := c[i].winningNumberCount()
		if score > 0 {
			channel <- score
			wg.Add(1)
			go getScratchCardCount(cards, i+1, i+1+score, wg, channel)
		}
	}
}
...
func (Solver) Solve(input string) string {
	...
	respChan := make(chan int, len(cards))
	wg.Add(1)
	go getScratchCardCount(&cards, 0, len(cards), &wg, respChan)
	go func() {
		wg.Wait()
		close(respChan)
	}()
