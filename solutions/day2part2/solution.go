...
func requiredAmount(game day2part1.Game) int {
	amounts := map[day2part1.Cube]int{}
	for _, ss := range game.Subsets {
		for _, ac := range ss.CubesAndAmounts {
			if ac.Amount > amounts[ac.Cube] {
				amounts[ac.Cube] = ac.Amount
			}
		}
	}
	req := amounts[day2part1.Red] * amounts[day2part1.Green] * amounts[day2part1.Blue]
	return req
}
...
func (Solver) Solve(input string) string {
	...
	for _, line := range lines {
		...
		go func() {
			...
			game := day2part1.ParseGame(line)
			respChan <- requiredAmount(game)
		}()
	}
