...
func ParseAlmanac(lines []string) Almanac {
	seeds, found := strings.CutPrefix(lines[0], "seeds:")
	if !found {
		panic(errors.New("parse error"))
	}
	almanac := Almanac{
		Seeds:                 utils.ToIntSlice(seeds, " "),
		SeedToSoil:            make([]Map, 0),
		SoilToFertilizer:      make([]Map, 0),
		FertilizerToWater:     make([]Map, 0),
		WaterToLight:          make([]Map, 0),
		LightToTemperature:    make([]Map, 0),
		TemperatureToHumidity: make([]Map, 0),
		HumidityToLocation:    make([]Map, 0),
	}
	var insert func(Map)
	for i := 1; i < len(lines); i++ {
		...
		if strings.Contains(lines[i], "map") {
			...
			switch name {
			case "seed-to-soil":
				insert = func(m Map) { almanac.SeedToSoil = append(almanac.SeedToSoil, m) }
			case "soil-to-fertilizer":
				insert = func(m Map) { almanac.SoilToFertilizer = append(almanac.SoilToFertilizer, m) }
			case "fertilizer-to-water":
				insert = func(m Map) { almanac.FertilizerToWater = append(almanac.FertilizerToWater, m) }
			case "water-to-light":
				insert = func(m Map) { almanac.WaterToLight = append(almanac.WaterToLight, m) }
			case "light-to-temperature":
				insert = func(m Map) { almanac.LightToTemperature = append(almanac.LightToTemperature, m) }
			case "temperature-to-humidity":
				insert = func(m Map) { almanac.TemperatureToHumidity = append(almanac.TemperatureToHumidity, m) }
			case "humidity-to-location":
				insert = func(m Map) { almanac.HumidityToLocation = append(almanac.HumidityToLocation, m) }
			default:
				panic(errors.New(fmt.Sprintf("unknown map %s", name)))
			}
			continue
		}
		if insert == nil {
			panic("insert function not set")
		}
		arr := utils.ToIntSlice(lines[i], " ")
		insert(Map{Destination: arr[0], Source: arr[1], Length: arr[2]})
	}
	return almanac
}
...
func getLowestLocation(almanac Almanac) int {
	var locations []int
	for _, seed := range almanac.Seeds {
		soil := GetMappedValue(seed, almanac.SeedToSoil)
		fertilizer := GetMappedValue(soil, almanac.SoilToFertilizer)
		water := GetMappedValue(fertilizer, almanac.FertilizerToWater)
		light := GetMappedValue(water, almanac.WaterToLight)
		temp := GetMappedValue(light, almanac.LightToTemperature)
		hum := GetMappedValue(temp, almanac.TemperatureToHumidity)
		location := GetMappedValue(hum, almanac.HumidityToLocation)
