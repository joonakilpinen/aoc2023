...
var limits = map[Cube]int{
	Red:   12,
	Green: 13,
	Blue:  14,
}

type CubeWithAmount struct {
	Amount int
	Cube   Cube
}

type Subset struct {
	CubesAndAmounts []CubeWithAmount
}
...
func (game Game) IsPossible() bool {
	for _, ss := range game.Subsets {
		for _, ca := range ss.CubesAndAmounts {
			if ca.Amount > limits[ca.Cube] {
				return false
			}
		}
	}
...
func parseSubset(line string) Subset {
	var cubesWithAmounts []CubeWithAmount
	for _, part := range strings.Split(line, ", ") {
		...
		cube := Cube(split[1])
		cubesWithAmounts = append(cubesWithAmounts, CubeWithAmount{Amount: amount, Cube: cube})
	}
	return Subset{CubesAndAmounts: cubesWithAmounts}
}

func parseSubsets(line string) []Subset {
	var subsets []Subset
	for _, subset := range strings.Split(line, ";") {
		subsets = append(subsets, parseSubset(strings.TrimSpace(subset)))
	}
	return subsets
}

var gameRegex = regexp.MustCompile("^Game (?P<GameId>\\d+): (?P<Subsets>.*)$")

func ParseGame(line string) Game {
	matches := gameRegex.FindStringSubmatch(line)
	idIdx := gameRegex.SubexpIndex("GameId")
	id, err := strconv.Atoi(matches[idIdx])
	if err != nil {
		panic(err)
	}
	subsetIdx := gameRegex.SubexpIndex("Subsets")
	subsetLine := matches[subsetIdx]
	subsets := parseSubsets(subsetLine)
